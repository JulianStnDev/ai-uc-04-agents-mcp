...
Aufruf:
    ...
    python agent.py --alle --laeufe 3 --ausgabe evals/laeufe/v1
"""
...
SYSTEM_PROMPT = """Du bist der Support-Agent der Habit-Tracker-App FocusFlow. Du bearbeitest ein Kundenticket, das per E-Mail eingegangen ist. Heute ist der 24.09.2026.
...
async def bearbeite_ticket(ticket: dict, run_id: str, runs_dir: Path = RUNS_DIR) -> dict:
    key = api_key_pruefen()
...
async def main():
    ...
    async def einer(t, r):
        async with sem:
            lauf = await bearbeite_ticket(t, r, ausgabe)
